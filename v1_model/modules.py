...
from .utils import gabor_kernel, gauss_kernel
...
class GaussianUntunedAvg(nn.Module):
    ...
    def initialize(self, sig_inh, k_inh):
        self.weight = torch.zeros((len(k_inh), 1, self.kernel_size[0], self.kernel_size[0]))
        for i in range(len(k_inh)):
            gaussk = gauss_kernel(sigma=sig_inh[i], ks=self.kernel_size[0])
            self.weight[i, 0, :, :] = gaussk * k_inh[i]
        self.weight = nn.Parameter(self.weight, requires_grad=False)


class GaussianTunedAvg(nn.Module):
    ...
    def forward(self, x):
        return F.conv2d(x, self.weight, None, self.stride, self.padding, groups=self.in_channels)

    def initialize(self):
        gaussk = gauss_kernel(sigma=self.sigma, ks=self.kernel_size[0])
        self.weight = nn.Parameter(gaussk.expand(self.in_channels, 1, -1, -1), requires_grad=False)


class V1_Model(nn.Module):
    def __init__(self, sf, theta, sigx, sigy, phase, k_inh, sig_inh,
                 k_exc=25, cs_ratio=1, o_d=1,
                 noise_scale=1, noise_mode=None, noise_level=1,
                 simple_channels=128, complex_channels=128, ksize=25, ksize_div=15, stride=4, input_size=336):
        super().__init__()
        ...
        self.k_exc = k_exc
        self.k_inh = k_inh
        self.sig_inh = sig_inh
        self.cs_ratio = cs_ratio
        self.do_div = (self.k_inh>0).sum()>0
        ...
        self.div = GaussianUntunedAvg(kernel_size=ksize_div, stride=1)
        self.div.initialize(k_inh=self.k_inh, sig_inh=self.sig_inh)

        self.div_norm = Identity()
    ...
    def forward(self, x):
        ...
        x = self.gabors_f(x)
        # Divisive normalization [Batch, out_channels, H/stride, W/stride]
        x = self.div_norm_f(x)
    ...
    def gabors_f(self, x):
        ...
        c = self.complex(torch.sqrt(s_q0[:, self.simple_channels:, :, :] ** 2 +
                                    s_q1[:, self.simple_channels:, :, :] ** 2) * self.cs_ratio / np.sqrt(2))
        s = self.simple(s_q0[:, 0:self.simple_channels, :, :])
        return self.gabors(self.k_exc * torch.cat((s, c), 1))

    def div_norm_f(self, x):
        if self.do_div:
            return self.div_norm(x / (self.o_d + self.div(torch.mean(x, dim=1, keepdim=True))))
        else:
            return self.div_norm(x)
